/**
 * SynthesisBridge - Helper Components
 *
 * Extracted UI components for the Synthesis Bridge module.
 */
...
import {
    ...
    Gauge, Activity, PlayCircle, Share2, Loader2
} from 'lucide-react';
...
export const BlueprintStat: React.FC<BlueprintStatProps> = ({ label, value, color, detail }) => (
...
export const DomainCard: React.FC<DomainCardProps> = ({ label, sub, icon: Icon, color, active, onClick }) => (
    <button
        onClick={onClick}
        className={cn(
...
interface ImplementationTerminalProps {
    protocols: ProtocolStep[];
    isDeploying: boolean;
}

export const ImplementationTerminal: React.FC<ImplementationTerminalProps> = ({ protocols, isDeploying }) => {
    ...
    const renderNode = (node: DirectoryNode, depth = 0) => {
        ...
        return (
            <div key={node.name} className="space-y-0.5">
                ...
                {Array.isArray(node.children) && node.children.map((child: DirectoryNode) => renderNode(child, depth + 1))}
            </div>
        );
    };
    ...
    return (
        <div className="p-6 bg-black/40 border border-white/5 rounded-[3rem] shadow-inner font-mono overflow-hidden flex flex-col group/tree relative">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 max-h-[400px] relative z-10">
                {data.structure.map((root: DirectoryNode) => renderNode(root))}
            </div>
...
export const ProposalQueue: React.FC = () => {
...
interface ImplementationDeckProps {
    data: TechnicalManifest;
    onDeploy: (d: TechnicalManifest) => void;
    onArchive: (d: TechnicalManifest) => void;
}

export const ImplementationDeck: React.FC<ImplementationDeckProps> = ({ data, onDeploy, onArchive }) => {
