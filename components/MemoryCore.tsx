...
import { 
    ...
    Filter, LayoutGrid, Boxes, Brain, Tag, Archive, Plus
} from 'lucide-react';
...
const ArtifactCard = ({ art, onDigitize, openHoloProjector, isIndexing }: any) => {
    return (
        <motion.div 
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            whileHover={{ scale: 1.02 }}
            className={`bg-[#0a0a0a] border border-[#222] p-5 rounded-2xl flex flex-col gap-4 relative group hover:border-[#9d4edd] transition-all shadow-xl ${isIndexing ? 'animate-pulse' : ''}`}
        >
            ...
            <div className="flex justify-between items-center mt-auto pt-4 border-t border-white/5">
                ...
                <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    {art.type === 'application/pdf' && (
                        <button onClick={(e) => onDigitize(art.id, e)} className="p-2 bg-[#22d3ee]/10 border border-[#22d3ee]/30 text-[#22d3ee] rounded-lg hover:bg-[#22d3ee] hover:text-black transition-all">
                            <Scan size={12} />
                        </button>
                    )}
                    ...
                    }} className="p-2 bg-[#9d4edd]/10 border border-[#9d4edd]/30 text-[#9d4edd] rounded-lg hover:bg-[#9d4edd] hover:text-black transition-all">
                        <Eye size={12} />
                    </button>
                </div>
            </div>
