...
const HolographicNode = ({ id, data: nodeData, selected, dragging }: NodeProps) => {
    ...
    return (
        <motion.div 
            ...
            style={{ backgroundColor: 'var(--bg-card-top)' }}
        >
            {drift > 40 && (
            ...
            <div className="mb-5 px-1">
                <div className="text-[14px] font-black font-mono uppercase tracking-[0.2em] text-[var(--text-main)] truncate leading-none mb-2">{renderSafe(data.label as any)}</div>
                <div className="text-[10px] font-mono uppercase tracking-widest text-[var(--text-muted)] h-4 truncate">{renderSafe(data.subtext as any)}</div>
            </div>
        </motion.div>
    );
};
...
const CinematicEdge = ({ id, sourceX, sourceY, targetX, targetY, style, markerEnd, data: edgeData }: EdgeProps) => {
    ...
    return (
        <>
            <path id={id} className="react-flow__edge-path" d={edgePath} strokeWidth={1} stroke="var(--border-main)" markerEnd={markerEnd} />
            <motion.path
                d={edgePath}
                fill="none"
                strokeWidth={2.5}
...
const FormalModelViewer = ({ model }: { model: any }) => {
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="p-10 bg-[var(--bg-card-top)] border border-[var(--border-main)] rounded-[3rem] space-y-10 shadow-2xl relative overflow-hidden group/mfr">
            ...
            <div className="flex items-center justify-between border-b border-[var(--border-main)] pb-6">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h3 className="text-xs font-black font-mono text-[var(--text-main)] uppercase tracking-[0.4em]">Explicit Problem Model (MFR)</h3>
                        <p className="text-[8px] text-[var(--text-muted)] font-mono uppercase tracking-widest mt-1">Sovereign Reasoning Layer // L0</p>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-3 gap-10">
                <div className="space-y-4">
                    <div className="flex items-center gap-3 text-[9px] font-black text-[var(--text-muted)] uppercase tracking-widest">
                        <Radio size={12} className="text-[#9d4edd]" /> Initial State
                    </div>
                    <div className="p-6 bg-black/5 border border-[var(--border-main)] rounded-2xl text-[11px] font-mono text-[var(--text-muted)] italic leading-relaxed h-32 overflow-y-auto custom-scrollbar">
            ...
            {model.actions && (
                <div className="pt-6 border-t border-[var(--border-main)]">
                    <div className="text-[9px] font-black text-[var(--text-muted)] uppercase tracking-widest mb-4 flex items-center gap-2">
                        <GitBranch size={12}/> Transition Schema (Actions)
                    </div>
                    <div className="grid grid-cols-4 gap-4">
                        {model.actions.slice(0, 4).map((a: any, i: number) => (
                            <div key={i} className="p-4 bg-white/[0.02] border border-[var(--border-main)] rounded-2xl group/act hover:bg-white/[0.04] transition-all">
                                <div className="text-[10px] font-black text-[var(--text-main)] uppercase mb-2 truncate">{a.name}</div>
                                <div className="text-[8px] font-mono text-[var(--text-muted)] leading-relaxed line-clamp-2 italic">Pre: {a.precondition}</div>
...
const ProcessVisualizerContent = () => {
    ...
    return (
        <div className="h-full w-full bg-[var(--bg-main)] flex flex-col relative overflow-hidden font-sans border border-[var(--border-main)] rounded-[3.5rem] shadow-[0_0_150px_rgba(0,0,0,1)] group/pv transition-colors duration-500">
            
            {/* Sector Header */}
            <div className="h-24 border-b border-[var(--border-main)] bg-[var(--bg-header)] backdrop-blur-3xl z-[60] flex items-center justify-between px-12 shrink-0 relative overflow-hidden">
                ...
                <div className="flex items-center gap-12">
                    <div className="flex items-center gap-6">
                        ...
                        <div>
                            <h1 className="text-xl font-black font-mono uppercase tracking-[0.5em] text-[var(--text-main)] leading-none mb-2">Process Logic</h1>
                            <span className="text-[10px] text-[var(--text-muted)] font-mono uppercase tracking-widest block">MFR Formal Synthesis // v9.1-ALPHA</span>
                        </div>
                    </div>
                    <div className="flex items-center gap-1.5 bg-black/5 p-2 rounded-2xl border border-[var(--border-main)] shadow-inner">
                        {[
                            ...
                            <button 
                                ...
                                className={`px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-3 transition-all whitespace-nowrap
                                    ${activeTab === tab.id ? 'bg-[#9d4edd] text-black shadow-lg shadow-[#9d4edd]/30' : 'text-[var(--text-muted)] hover:text-[var(--text-main)] hover:bg-black/5'}
                                `}
                            >
                ...
                <div className="flex items-center gap-6">
                    <button onClick={handleAutoOrganize} className="p-3.5 bg-black/5 border border-[var(--border-main)] hover:border-[#10b981] rounded-2xl text-[var(--text-muted)] transition-all active:scale-90" title="Autopoietic Organization">
                        <RefreshCw size={22} />
                    </button>
            ...
            <div className="flex-1 relative overflow-hidden bg-transparent z-10">
                <AnimatePresence mode="wait">
                    {activeTab === 'living_map' && (
                        <motion.div key="map" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="h-full relative">
                            <ReactFlow nodes={nodes} edges={edges} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange} onConnect={onConnect} nodeTypes={nodeTypes} edgeTypes={edgeTypes} fitView>
                                <Background color="var(--border-main)" gap={60} size={1.5} variant={BackgroundVariant.Dots} style={{ opacity: 0.3 }} />
                                <Controls />
                                <MiniMap />
                            </ReactFlow>
                        </motion.div>
                    ...
                    {activeTab === 'architect' && (
                        <motion.div key="architect" initial={{ opacity: 0, scale: 1.05 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="h-full flex flex-col items-center justify-start p-20 bg-transparent overflow-y-auto custom-scrollbar relative">
                            ...
                            <div className="w-full max-w-[1400px] space-y-16 relative z-10">
                                <div className="bg-[var(--bg-card-top)] backdrop-blur-3xl border border-[var(--border-main)] rounded-[4rem] p-16 shadow-2xl flex flex-col gap-12">
                                    <div className="flex flex-col gap-6">
                                        ...
                                        <textarea 
                                            ...
                                            className="w-full h-40 bg-black/5 border border-[var(--border-main)] rounded-[2rem] p-10 text-lg font-mono text-[var(--text-main)] outline-none focus:border-[#9d4edd]/60 resize-none transition-all placeholder:text-gray-400 shadow-inner" 
                                        />
                                    </div>
                    ...
                    {activeTab === 'workflow' && (
                        <motion.div key="workflow" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="h-full overflow-y-auto custom-scrollbar p-16 flex gap-12">
                                ...
                                <div className="space-y-4">
                                    <div className="flex items-center justify-between px-2 mb-2">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <span className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-widest">Operational Sequence Protocols</span>
                                        </div>
                                        {processData.coherenceScore && (
                                            <div className="flex items-center gap-2">
                                                <span className="text-[10px] font-mono text-[var(--text-muted)] uppercase tracking-widest">Coherence Index</span>
                                    ...
                                    {processData.generatedWorkflow?.protocols?.map((p: any, i: number) => (
                                        <motion.div 
                                            ...
                                            className="p-8 bg-[var(--bg-card-top)] border border-[var(--border-main)] rounded-3xl flex items-center justify-between group hover:border-[#9d4edd]/30 transition-all shadow-xl"
                                        >
                                            <div className="flex items-center gap-8">
                                                <div className="w-14 h-14 rounded-2xl bg-black/5 border border-[var(--border-main)] flex items-center justify-center font-mono font-black text-2xl text-[#9d4edd] group-hover:bg-[#9d4edd]/10 transition-colors">{i+1}</div>
                                                <div>
                                                    <div className="text-sm font-bold text-[var(--text-main)] uppercase font-mono tracking-tight group-hover:text-[#9d4edd] transition-colors">{p.instruction}</div>
                                                    <div className="flex items-center gap-3 mt-2">
                                                        <div className="px-3 py-1 rounded-full bg-black/5 border border-[var(--border-main)] text-[9px] font-mono text-[var(--text-muted)] uppercase font-black tracking-widest">{p.role}</div>
                                                        <div className="flex items-center gap-2 text-[9px] font-mono text-[var(--text-muted)] uppercase">
                                                            <Target size={10} /> {p.modelConstraintRef}
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                            <button onClick={() => handleExecuteStep(i)} className="p-4 bg-black/5 hover:bg-[#9d4edd] text-[var(--text-muted)] hover:text-black rounded-2xl transition-all shadow-xl active:scale-90">
                            ...
                            {/* Sidepanel: Retrieved Information Stack */}
                            <div className="w-80 flex flex-col gap-6 shrink-0">
                                <div className="bg-[var(--bg-card-top)] border border-[var(--border-main)] rounded-[2.5rem] p-8 shadow-2xl relative overflow-hidden group">
                                    <div className="flex items-center gap-3 mb-6 relative z-10">
                                        <Boxes size={18} className="text-[#22d3ee] animate-pulse" />
                                        <span className="text-[10px] font-black text-[var(--text-main)] uppercase tracking-[0.4em]">Retrieved Info</span>
                                    </div>
                                    <div className="flex flex-col-reverse gap-2 max-h-[400px] overflow-y-auto custom-scrollbar px-2 pb-2">
                                        {[...Array(6)].map((_, i) => (
                                            <motion.div 
                                                ...
                                                transition={{ delay: i * 0.1 }}
                                                className="h-10 bg-black/5 border border-[var(--border-main)] rounded-lg flex items-center px-4 gap-3 group/box hover:bg-[#22d3ee]/10 hover:border-[#22d3ee]/30 transition-all cursor-pointer"
                                            >
                                                <div className="w-2 h-2 rounded-sm bg-[#22d3ee] shadow-[0_0_8px_#22d3ee] group-hover/box:scale-110 transition-transform" />
                                                <div className="text-[8px] font-mono text-[var(--text-muted)] group-hover/box:text-[var(--text-main)] uppercase tracking-tighter truncate">
                                    ...
                                    <div className="mt-8 pt-6 border-t border-[var(--border-main)] text-center">
                                        <p className="text-[9px] font-mono text-[var(--text-muted)] uppercase tracking-widest leading-relaxed">
                                            Grounding data stacked via Hybrid Retrieval Bridge.
                                        </p>
                                    </div>
                                </div>

                                <div className="bg-[var(--bg-card-top)] border border-[var(--border-main)] rounded-[2.5rem] p-8 shadow-2xl flex flex-col justify-between h-64 relative overflow-hidden group">
                                    ...
                                    <div>
                                        <div className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-[0.4em] mb-4">Lattice Status</div>
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div className="flex justify-between text-[8px] font-mono text-[var(--text-muted)] uppercase tracking-widest">
                    ...
                    {activeTab === 'diagram' && (
                        <motion.div key="diagram" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="h-full p-20 bg-transparent">
