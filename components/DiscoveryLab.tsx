...
import { 
    ...
    ChevronRight, ArrowRight, Minimize2, Trash2, CheckCircle2
} from 'lucide-react';
...
const DiscoveryLab: React.FC = () => {
    ...
    const activeKnowledge = useMemo(() => {
        ...
        return nodes;
    }, [research.tasks, discovery.hypotheses, axioms]);
    ...
    return (
        <div className="h-full w-full bg-[#030303] flex flex-col relative overflow-hidden font-sans">
            <SuperLattice nodes={activeKnowledge} mode="AMBIENT" />
                ...
                <div className="w-1/3 flex flex-col gap-4">
                    ...
                    <div className="flex-1 bg-[#050505]/80 backdrop-blur-xl border border-[#333] rounded-xl overflow-hidden flex flex-col shadow-2xl">
                        <div className="p-3 border-b border-[#333] bg-[#0a0a0a]/50 flex justify-between items-center text-[10px] font-mono text-gray-500 uppercase tracking-widest">
                            <Activity size={14} className="text-[#f59e0b]" /> Active Probes
                        </div>
                        <div className="flex-1 overflow-y-auto p-3 space-y-3 custom-scrollbar">
                            {research.tasks.filter(t => t.status !== 'COMPLETED').map(task => (
                                <div key={task.id} className="bg-[#111] border border-[#222] p-3 rounded-lg group">
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
                            {axioms.map(axiom => (
                                <div key={axiom.id} className="p-4 bg-black/40 border border-[#9d4edd]/30 rounded-xl relative group hover:border-[#9d4edd] transition-all">
                                    ...
                                    <p className="text-[11px] text-gray-300 leading-relaxed font-mono italic">"{axiom.statement}"</p>
                                    <div className="mt-2 flex items-center gap-2">
                                        <CheckCircle2 size={10} className="text-[#42be65]" />
                                        <span className="text-[8px] text-gray-600 font-mono">SUPPORTED BY {axiom.sourceNodes.length} NODES</span>
                                    </div>
                                </div>
                            ))}
                            {axioms.length === 0 && (
