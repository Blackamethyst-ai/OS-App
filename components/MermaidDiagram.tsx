  ...
  if (error) {
    return (
      <div className="w-full h-full flex flex-col items-center justify-center bg-red-900/10 p-8 border border-red-500/20 rounded-xl relative overflow-hidden">
        ...
        <button onClick={handleAutoRepair} disabled={isRepairing} className="px-6 py-2 bg-red-900/20 hover:bg-red-900/40 border border-red-500/50 rounded uppercase tracking-wider text-xs font-mono text-red-200 flex items-center gap-2 mb-6">
        ...
        <div className="w-full max-w-2xl bg-black border border-red-900/50 p-4 rounded overflow-auto max-h-64 shadow-inner"><code className="text-[10px] font-mono text-red-300 whitespace-pre-wrap">{activeCode}</code></div>
      </div>
    );
  }
  ...
  return (
    <div className="w-full h-full bg-[#030303] relative overflow-hidden group border border-[#1f1f1f] rounded-lg">
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-3 z-50 opacity-0 group-hover:opacity-100 transition-all">
         <div className="flex bg-[#0a0a0a]/90 border border-[#333] rounded-full p-2">
             <button onClick={() => setTransform(p => ({...p, scale: Math.max(0.1, p.scale - 0.2)}))} className="p-2 text-gray-400 hover:text-white"><ZoomOut className="w-4 h-4"/></button>
             ...
             <button onClick={() => setTransform(p => ({...p, scale: Math.min(10, p.scale + 0.2)}))} className="p-2 text-gray-400 hover:text-white"><ZoomIn className="w-4 h-4"/></button>
         </div>
      </div>
    </div>
  );
