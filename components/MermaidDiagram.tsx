...
import { RefreshCw, Image as ImageIcon, FileCode, Maximize, ZoomIn, ZoomOut, Move, Download, AlertCircle, Focus } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    const render = async () => {
      ...
      try {
        ...
        if (isMounted) {
           setError("Syntax Error: The generated diagram structure contains invalid Mermaid.js syntax.");
        }
      } finally {
  ...
  if (error) {
    return (
      <div className="w-full h-full flex flex-col items-center justify-center bg-red-900/10 p-8 border border-red-500/20 rounded-xl relative overflow-hidden">
        ...
        <p className="text-gray-500 font-mono text-xs max-w-md text-center mb-6">The structural integrity of the generated diagram code was compromised. Attempting to render raw data for manual inspection.</p>
