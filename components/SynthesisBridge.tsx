...
const BlueprintStat = ({ label, value, color, detail }: { label: string, value: string, color: string, detail?: string }) => (
    <div className="flex flex-col gap-1 p-4 bg-white/[0.02] border border-white/5 rounded-2xl relative overflow-hidden group hover:border-white/10 transition-all shadow-inner glass-refraction">
...
const DomainCard = ({ label, sub, icon: Icon, color, active, onClick }: any) => (
    <button 
        onClick={onClick}
        className={cn(
            "w-full p-4 rounded-2xl border transition-all text-left flex items-center gap-4 relative overflow-hidden group/domain",
    ...
    >
        ...
        <div className={cn(
            "w-10 h-10 rounded-xl border flex items-center justify-center transition-all duration-500 shrink-0",
            active ? "bg-current border-current text-black shadow-lg" : "bg-black/40 border-white/10 text-gray-600"
        )}>
            <Icon size={18} className="group-hover/domain:scale-110 transition-transform" />
        </div>
        ...
        {active && (
            <div className="w-1 h-1 rounded-full bg-current animate-pulse shadow-[0_0_8px_currentColor] ml-2" />
        )}
    </button>
);
...
const TreeView = ({ data }: { data: TechnicalManifest }) => {
    ...
    const renderNode = (node: DirectoryNode, depth = 0) => {
        ...
        return (
            <div key={node.name} className="space-y-0.5">
                <div 
                    className="flex items-center gap-3 py-2 hover:bg-white/5 rounded-xl px-4 transition-all cursor-default group/node border border-transparent hover:border-white/5"
                    style={{ paddingLeft: `${depth * 1.5 + 0.8}rem` }}
                >
                    <div className={cn(
                        "w-6 h-6 rounded-lg flex items-center justify-center border transition-all shadow-lg",
                    ...
                    <div className="flex flex-col min-w-0">
                        ...
                        {node.description && (
                            <div className="flex items-center gap-2 mt-0.5 min-w-0">
                                <span className="text-[7px] text-gray-600 font-mono truncate max-w-[280px] uppercase">{node.description}</span>
                                <div className="h-px w-3 bg-white/5 shrink-0" />
                    ...
                    <div className="ml-auto flex items-center gap-3 opacity-0 group-hover/node:opacity-100 transition-opacity">
                        {node.securityAttestation === 'VERIFIED' && (
                            <div className="flex items-center gap-1 px-1.5 py-0.5 bg-[#10b981]/10 rounded border border-[#10b981]/20">
                                <ShieldCheck size={8} className="text-[#10b981]" />
                            </div>
                        )}
                        <span className="text-[7px] font-mono text-gray-700 uppercase tracking-widest whitespace-nowrap">{node.modified || '2025.Q1'}</span>
                    </div>
                </div>
    ...
    return (
        <div className="p-8 bg-black/40 border border-white/5 rounded-[3rem] shadow-inner font-mono overflow-hidden flex flex-col group/tree relative">
            ...
            <div className="flex items-center justify-between mb-8 px-2 relative z-10">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-[7px] text-gray-500 font-mono uppercase tracking-[0.3em] mt-1 block">Synthesis v9.5 //PARA 2.0</p>
                    </div>
                </div>
            </div>
            
            <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 max-h-[500px] relative z-10">
                {data.structure.map((root: any) => renderNode(root))}
            </div>

            <div className="mt-8 pt-6 border-t border-white/5 flex justify-between items-center px-2 opacity-40 group-hover/tree:opacity-100 transition-opacity relative z-10">
                <div className="flex gap-4 text-[7px] font-mono text-gray-600 uppercase tracking-[0.2em] font-black">
                    <span>Protocol: IMPERIAL_PARA</span>
                    <span>Class: Recursive_Lattice</span>
                </div>
                <button className="text-[8px] font-black font-mono text-[#f1c21b] hover:text-white transition-colors uppercase tracking-widest flex items-center gap-1.5">
                    <FileText size={10} /> View Raw Manifest
                </button>
            </div>
        </div>
    );
...
const ImplementationDeck: React.FC<{
    ...
    return (
        ...
        >
            <div className="p-10 crystalline rounded-[4rem] relative overflow-hidden shadow-2xl glass-refraction">
                <div className="absolute top-0 right-0 p-10 opacity-[0.01] rotate-12 pointer-events-none scale-150"><Component size={180} /></div>
                
                <div className="flex justify-between items-start mb-10 relative z-10">
                    <div className="space-y-4">
                        <div className="flex items-center gap-3">
                            <div className="px-3 py-1 bg-[#10b981]/10 border border-[#10b981]/30 rounded-full flex items-center gap-2 backdrop-blur-xl">
                                <ShieldCheck size={10} className="text-[#10b981]" />
                                <span className="text-[9px] font-black text-[#10b981] uppercase tracking-[0.3em]">Imperial Verifier Lock</span>
                            </div>
                            <span className="text-[8px] font-mono text-gray-600 uppercase tracking-[0.4em] font-black opacity-60">ID_REF: {crypto.randomUUID().split('-')[0].toUpperCase()}</span>
                        </div>
                        <h2 className="text-5xl font-black text-white font-mono tracking-tighter uppercase leading-[0.9]">{data.title}</h2>
                    </div>

                    <div className="flex gap-3 shrink-0">
                        <button onClick={() => onArchive(data)} className="p-4 bg-white/5 border border-white/10 rounded-2xl text-gray-500 hover:text-white transition-all hover:bg-white/10 hover:border-white/30 shadow-2xl active:scale-95">
                            <Save size={20} />
                        </button>
                        <div className="px-6 py-3 bg-black/40 border border-white/5 rounded-2xl flex flex-col items-end shadow-inner min-w-[140px]">
                            <span className="text-[8px] font-black text-gray-600 uppercase tracking-widest mb-1 opacity-50">Risk Gradient</span>
                            <span className={cn(
                                "text-xl font-black font-mono tracking-widest",
                ...
                <div className="p-8 bg-black/60 border border-white/5 rounded-[3rem] shadow-inner group/logic relative overflow-hidden mb-8">
                    <div className="absolute top-0 right-0 p-6 opacity-[0.02] group-hover/logic:opacity-08 transition-opacity duration-1000"><Microscope size={80} /></div>
                    <div className="flex items-center gap-3 mb-6">
                        <Terminal size={18} className="text-[#7B2CFF]" />
                        <span className="text-[10px] font-black text-gray-500 uppercase tracking-[0.4em]">Operational Directive</span>
                    </div>
                    <p className="text-2xl text-gray-300 font-mono leading-tight italic border-l-4 border-[#7B2CFF] pl-10 group-hover:text-white transition-colors duration-1000">
            ...
            <div className="grid grid-cols-12 gap-8">
                <div className="col-span-8 space-y-6">
                    <div className="flex items-center justify-between px-4 mb-2">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <span className="text-lg font-black text-white uppercase tracking-[0.4em]">Deployment Protocol</span>
                                <p className="text-[9px] text-gray-600 font-mono uppercase tracking-widest mt-1 opacity-60">Zero-drift execution sequence</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-4 pt-4">
                        {(data.protocols || []).map((step: any, i: number) => (
                            <motion.div 
                                ...
                                className="p-8 bg-[#0a0a0c] border border-white/5 rounded-[3.5rem] flex items-center gap-10 group hover:border-[#10b981]/40 transition-all shadow-2xl relative overflow-hidden backdrop-blur-4xl"
                            >
                                <div className="w-14 h-14 bg-black border border-white/10 rounded-2xl flex items-center justify-center shrink-0 group-hover:bg-[#10b981] group-hover:text-black transition-all shadow-2xl relative z-10 overflow-hidden group-hover:scale-105">
                                    <span className="text-xl font-black font-mono">{(i+1).toString().padStart(2, '0')}</span>
                                </div>
                                <div className="flex-1 min-w-0 relative z-10">
                                    <div className="text-[9px] font-black text-[#10b981] uppercase tracking-[0.4em] mb-2 opacity-60 group-hover:opacity-100 flex items-center gap-3">
                                        {step.phase || step.role || 'CORE_LOGIC'}
                                        <div className="h-px w-8 bg-current opacity-20" />
                                    </div>
                                    <p className="text-lg text-gray-300 font-mono leading-relaxed group-hover:text-white transition-colors uppercase tracking-tight">{step.instruction}</p>
                                </div>
                                <div className="px-5 py-2.5 bg-black/60 border border-white/10 rounded-xl flex items-center gap-4 shrink-0 relative z-10 shadow-inner group-hover:border-white/30 transition-all">
                                    <Binary size={16} className="text-gray-600 group-hover:text-[#22d3ee] transition-colors" />
                                    <span className="text-[10px] font-mono text-gray-500 font-black uppercase tracking-[0.3em]">{step.nodeRef || 'STABLE'}</span>
                                </div>
                            </motion.div>
                        ))}
                    </div>
                ...
                <div className="col-span-4 flex flex-col gap-8">
                    <div className="bg-[#0a0a0c] border border-white/5 rounded-[4rem] p-10 flex flex-col gap-10 shadow-2xl h-full invisible-glass backdrop-blur-4xl relative overflow-hidden">
                        ...
                        <div className="flex items-center gap-5 relative z-10">
                            <div className="p-4 bg-[#7B2CFF]/10 rounded-2xl text-[#7B2CFF] border border-[#7B2CFF]/30 shadow-2xl">
                                <Compass size={32} />
                            </div>
                            <div>
                                <span className="text-base font-black text-white uppercase tracking-[0.4em]">Lattice Meta</span>
                                <p className="text-[9px] text-gray-500 font-mono uppercase mt-1.5 tracking-widest opacity-60">Success probability model</p>
                            </div>
                        </div>
                        ...
                        <div className="flex-1 flex flex-col gap-12 relative z-10">
                            <div className="space-y-6">
                                <div className="flex justify-between items-end px-1">
                                    <span className="text-[10px] font-black text-gray-600 uppercase tracking-widest">Confidence Rating</span>
                                    <span className="text-xl font-black font-mono text-[#7B2CFF] leading-none">{data.viability}%</span>
                                </div>
                                <div className="h-2 w-full bg-black/60 rounded-full overflow-hidden border border-white/5 shadow-inner p-px">
                                    <motion.div 
                                        ...
                                        className="h-full rounded-full bg-gradient-to-r from-[#7B2CFF] to-[#18E6FF] shadow-[0_0_20px_rgba(123,44,255,0.3)]" 
                                    />
                                </div>
                            </div>
                            ...
                            <div className="space-y-5 px-2">
                                {[
                                    ...
                                    <div key={idx} className="flex justify-between items-center py-3 border-b border-white/5 last:border-0 group/stat">
                                        <div className="flex items-center gap-3">
                                            <stat.icon size={12} className="text-gray-700 group-hover/stat:text-white transition-colors" />
                                            <span className="text-[9px] font-mono text-gray-500 uppercase tracking-widest group-hover/stat:text-gray-300">{stat.label}</span>
                                        </div>
                                        <span className="text-[11px] font-black font-mono uppercase" style={{ color: stat.color }}>{stat.val}</span>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="mt-auto pt-10 border-t border-white/5 flex flex-col gap-5">
                                <button 
                                    ...
                                    className={cn(
                                        "w-full py-5 rounded-[2.5rem] text-[11px] font-black uppercase tracking-[0.6em] transition-all flex items-center justify-center gap-6 shadow-inner active:scale-95 group",
                                        isDeploying ? "bg-gray-900 text-gray-600 cursor-default" : "bg-[#10b981] hover:bg-[#15d192] text-black shadow-[0_20px_60px_rgba(16,185,129,0.3)]"
                                    )}
                                >
                                    {isDeploying ? <Loader2 size={24} className="animate-spin" /> : <PlayCircle size={28} className="group-hover:rotate-90 transition-transform duration-1000 fill-current" />}
                                    {isDeploying ? 'Protocol Live' : 'Commit Protocol'}
                                </button>
                                <button className="w-full py-4 border border-white/10 text-gray-500 hover:text-white rounded-[2.5rem] text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-4 hover:bg-white/5 active:scale-95 group">
                                    <Share2 size={20} className="group-hover:scale-110 transition-transform" /> Synchronize
                                </button>
                            </div>
                        </div>
                    </div>
...
const SynthesisBridge: React.FC = () => {
    ...
    const PRESETS = [
        { id: 'para_ritual', label: 'PARA 2.0 Imperial Ritual', type: 'DRIVE', description: 'Architect a Tier-1 recursive PARA file hierarchy with semantic Zettelkasten linking and strict convention protocols.', icon: FolderTree },
        { id: 'lattice_infra', label: 'Sovereign Cloud Topology', type: 'SYSTEM', description: 'Forge a high-fidelity cloud manifest featuring edge data refraction and self-healing node clusters.', icon: Globe },
        { id: 'ts_fortress', label: 'Type Sovereignty', type: 'CODE', description: 'Imperial protocol for React/TypeScript structural integrity. Enforces explicit generic inheritance.', icon: Shield },
    ];
    ...
    return (
        <div className="h-full w-full bg-transparent flex flex-col font-sans overflow-hidden transition-all duration-700">
            ...
            <div className="flex-1 flex overflow-hidden p-10 gap-10 relative z-10">
                <div className="w-[400px] flex flex-col gap-8 shrink-0 overflow-y-auto custom-scrollbar pr-3">
                    <div className="p-8 bg-[#0a0a0c]/60 border border-white/5 rounded-[3rem] shadow-2xl backdrop-blur-3xl relative overflow-hidden group/sector">
                        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(123,44,255,0.02)_0%,transparent_70%)]" />
                        <div className="flex items-center gap-3 mb-6 px-1 relative z-10">
                            <Target size={18} className="text-[#7B2CFF] animate-pulse" />
                            <span className="text-[12px] font-black text-gray-500 uppercase tracking-[0.5em]">Forge Sector</span>
                        </div>
                    ...
                    <div className="p-8 bg-[#0a0a0c]/60 border border-white/5 rounded-[3rem] shadow-2xl backdrop-blur-3xl relative overflow-hidden group/ritual">
                        <div className="absolute inset-0 bg-[radial-gradient(circle_at_bottom_right,rgba(255,255,255,0.01)_0%,transparent_70%)]" />
                        <div className="flex items-center gap-3 mb-6 px-1 relative z-10">
                            <BookOpen size={18} className="text-gray-500" />
                            <span className="text-[12px] font-black text-gray-500 uppercase tracking-[0.5em]">Imperial Rituals</span>
                        </div>
                        <div className="space-y-4 relative z-10">
                            {PRESETS.map(preset => (
                                <button 
                                    ...
                                    className={cn(
                                        "w-full p-5 rounded-2xl text-left transition-all group border shadow-xl flex flex-col gap-2 relative overflow-hidden",
                                ...
                                >
                                    ...
                                    <div className="text-[9px] text-gray-600 font-mono line-clamp-2 uppercase leading-relaxed opacity-60 group-hover:opacity-100 transition-opacity">{preset.description}</div>
                                </button>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="space-y-5 pt-1">
                        <textarea 
                            ...
                            className="w-full h-36 bg-black/60 border border-white/5 rounded-[2.5rem] p-6 text-[11px] font-mono text-gray-300 outline-none focus:border-[#7B2CFF]/60 transition-all placeholder:text-gray-800 shadow-inner resize-none"
                        />
                        <button 
                            onClick={() => generateBlueprint()}
                            disabled={isGenerating}
                            className="w-full py-5 bg-[#7B2CFF] hover:bg-[#8e49ff] text-black rounded-[2.5rem] text-[11px] font-black uppercase tracking-[0.5em] transition-all shadow-[0_20px_60px_rgba(123,44,255,0.3)] active:scale-95 disabled:opacity-50 flex items-center justify-center gap-6 group/gen"
                        >
                            {isGenerating ? <Loader2 className="w-6 h-6 animate-spin" /> : <RefreshCw size={22} className="group-hover/gen:rotate-180 transition-transform duration-700" />}
                        ...
                        {result ? (
                            ...
                            >
                                <div className="relative">
                                    <Aperture size={200} className="text-white animate-[spin_20s_linear_infinite]" />
