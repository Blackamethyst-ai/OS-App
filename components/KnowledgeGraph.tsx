...
import { BrainCircuit, X, Search, GitBranch, Sparkles } from 'lucide-react';
...
const KnowledgeGraph: React.FC<KnowledgeGraphProps> = ({ nodes, onNodeClick }) => {
        ...
        const loop = () => {
            ...
            simulationNodes.forEach(n => {
                ...
                if (isSelected || isHovered || isMatch) {
                    ...
                    ctx.fillText(n.label || 'Node', n.x, n.y - 15);
                }
            });
    ...
    const handleBranch = () => {
        ...
        addResearchTask({
            ...
            query: `Lattice Branch: Investigation into "${activeNode.label}"`,
            status: 'QUEUED',
            progress: 0,
            logs: [`Branched from knowledge graph node: ${activeNode.id}`],
            timestamp: Date.now()
        });
        addLog('SUCCESS', `LATTICE_BRANCH: Dispatching probe for node "${activeNode.label}"`);
        setSelectedNodeId(null);
    };
        ...
        <div className="relative w-full h-full bg-[#050505] rounded-xl overflow-hidden border border-white/5">
            <div className="absolute top-4 left-4 z-10 flex flex-col gap-1 pointer-events-none">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold font-mono text-white uppercase tracking-widest">Neural Topology</span>
                </div>
            </div>
            ...
            <AnimatePresence>
                {selectedNodeId && activeNode && (
                    <motion.div 
                        ...
                        exit={{ opacity: 0, x: 20 }}
                        className="absolute top-4 right-4 w-72 bg-black/80 backdrop-blur-md border border-[#333] p-6 rounded-2xl shadow-2xl z-20"
                    >
                        <div className="flex justify-between items-start mb-4">
                            <div>
                                ...
                                <span className="text-[8px] font-mono text-gray-600 uppercase tracking-widest">ID: {activeNode.id.substring(0,8)}</span>
                            </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="pt-4 border-t border-white/5 flex flex-col gap-2">
                                <button onClick={handleBranch} className="w-full py-2 bg-[#9d4edd]/20 border border-[#9d4edd]/40 text-[#9d4edd] rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-[#9d4edd] hover:text-black transition-all flex items-center justify-center gap-2">
                                    <GitBranch size={12} /> Branch Probe
                                </button>
                            </div>
                        </div>
                    </motion.div>
