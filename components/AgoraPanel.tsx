    ...
    const startSimulation = async () => {
        ...
        try {
            ...
            for (let i = 0; i < 9; i++) {
                ...
                if (turn.newMindset) {
                    currentPersonas[speakerIndex] = { ...speaker, currentMindset: turn.newMindset };
    ...
    const toggleVoice = async () => {
        ...
        try {
            ...
            await liveSession.connect('Puck', {
                systemInstruction: "You are the Voice of the Chair (Moderator). The user is the Chair. Listen to their commands. If they want to interrupt, direct an agent, or change the topic, use the 'inject_intervention' tool to send a directive.",
                tools: [interveneTool]
            });
            ...
            liveSession.onToolCall = async (name, args) => {
                if (name === 'inject_intervention') {
                    ...
                    let targetId = 'ALL';
                    if (targetName !== 'ALL') {
                        const match = personas.find(p => p.name.toUpperCase().includes(targetName) || p.role.includes(targetName));
                        if (match) targetId = match.id;
                    }
                    pendingWhispers.current[targetId] = msg;
                    setLastIntervention({ target: targetName, msg });
                    setTimeout(() => setLastIntervention(null), 4000);
                    ...
                    {status === 'IDLE' ? (
                        ...
                        <div className="relative w-full h-full flex items-center justify-center">
                                ...
                                {personas.length > 0 && (
                                    <ResponsiveContainer width="100%" height="100%">
                                        <RadarChart cx="50%" cy="50%" outerRadius="70%" data={radarData}>
                                            ...
                                            {personas.map((p) => (
                                                <RechartRadar key={p.id} name={p.name} dataKey={p.id} stroke={p.avatar_color} strokeWidth={2} fill={p.avatar_color} fillOpacity={0.1} isAnimationActive={false} />
                                            ))}
                                        </RadarChart>
                                    </ResponsiveContainer>
                                )}
                            ...
                            {personas.map((p, i) => {
                                ...
                                return (
                                    <motion.div key={p.id} initial={{ scale: 0, opacity: 0 }} animate={{ scale: isActive ? 1.1 : 1, opacity: 1, x, y }} className="absolute w-32 flex flex-col items-center justify-center z-10 cursor-pointer group" onContextMenu={(e) => handleWhisper(p.id, e)}>
                                        <div className="relative">
                                            <div className={`w-14 h-14 rounded-full border-2 flex items-center justify-center bg-[#111] transition-all duration-300 relative overflow-hidden ${isActive ? 'shadow-[0_0_30px_var(--glow)] border-[var(--glow)]' : 'border-gray-700'}`} style={{ '--glow': p.avatar_color } as any}>
                                                <span className="text-lg font-bold" style={{ color: p.avatar_color }}>{p.name.charAt(0)}</span>
                                                {isActive && <div className="absolute inset-0 bg-white/10 animate-ping rounded-full"></div>}
                                                {isActive && !isNarrationMuted && (
                                                    <div className="absolute bottom-0 left-0 right-0 flex justify-center items-end gap-0.5 h-6 opacity-60">
                                                        {[1,2,3,4,3,2,1].map((n, idx) => (
                                                            <motion.div key={idx} animate={{ height: [4, 12, 4] }} transition={{ duration: 0.5, repeat: Infinity, delay: idx * 0.1 }} className="w-1 bg-white" />
                                                        ))}
                                                    </div>
                                                )}
                                            </div>
                                        ...
                                        <div className="mt-2 text-center">
                                            <div className="text-[10px] font-bold text-gray-300 bg-black/50 px-2 rounded backdrop-blur">{p.name}</div>
                                            <div className="text-[8px] font-mono text-gray-500 uppercase mt-0.5">{p.role}</div>
                                        </div>
                                        <div className="mt-1 w-16 h-1 bg-[#222] rounded-full overflow-hidden">
                                            <motion.div className="h-full transition-all duration-700 ease-out" initial={{ width: 0 }} animate={{ width: `${p.currentMindset.alignment}%` }} style={{ backgroundColor: p.avatar_color }} />
                                        </div>
                                    </motion.div>
                                );
                            })}
                            {status === 'DEBATING' && (
                                <div className="absolute bottom-8 left-8 right-8 h-24 bg-[#0a0a0a]/80 backdrop-blur border border-[#333] rounded-lg p-3 flex flex-col justify-end pointer-events-none">
                                    <div className="flex justify-between items-end mb-2">
                                        <span className="text-[9px] font-mono text-gray-500 uppercase flex items-center gap-2"><Activity className="w-3 h-3" /> Constructive Tension</span>
            ...
            <div className="w-1/2 flex flex-col bg-[#080808]">
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-4" ref={scrollRef}>
                    {history.map((turn) => {
                        ...
                        return (
                            <motion.div key={turn.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className={`text-xs font-mono flex flex-col gap-1 transition-opacity ${isSpeaking ? 'opacity-100' : 'opacity-70'}`}>
                                ...
                                <div className={`bg-[#111] border p-3 rounded-tr-lg rounded-b-lg text-gray-300 leading-relaxed relative ${isSpeaking ? 'border-[#9d4edd]' : 'border-[#222]'}`}>
                                    {turn.newMindset && (
                                        <div className="absolute right-2 top-2 flex gap-1">
                                            ...
                                            {turn.newMindset.skepticism > 60 && <div className="w-1.5 h-1.5 rounded-full bg-[#ef4444]"></div>}
                                        </div>
                                    )}
                                    {turn.text}
                                </div>
                    ...
                    {status === 'DEBATING' && !activeSpeakerId && (
                        <div className="flex items-center gap-2 text-xs text-gray-500 animate-pulse mt-4 pl-2"><Loader2 className="w-3 h-3 animate-spin" /><span className="font-mono">Processing response...</span></div>
                    )}
                </div>
                {report && (
                    <motion.div initial={{ y: 100 }} animate={{ y: 0 }} className="h-1/2 border-t border-[#1f1f1f] bg-[#0c0c0c] p-6 flex flex-col">
