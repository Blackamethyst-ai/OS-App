...
import { Users, Loader2, MessageSquare, AlertCircle, CheckCircle, Mic, Zap, Activity, GitCommit, Save, Layers, ArrowUpRight, Radio } from 'lucide-react';
...
const AgoraPanel: React.FC<AgoraPanelProps> = ({ artifact }) => {
    ...
    const startSimulation = async () => {
        ...
        try {
            ...
            // Local copy of personas to update state during loop without waiting for React render
            ...
            for (let i = 0; i < 9; i++) {
                ...
                // Check for whispers (from Voice or Context Menu)
                // First check specific whisper, then check broadcast ('ALL')
                ...
                // Update Mental State locally and in State
                if (turn.newMindset) {
                    currentPersonas[speakerIndex] = {
                        ...speaker,
                        currentMindset: turn.newMindset
                    };
                    setPersonas([...currentPersonas]); // Trigger re-render of chart
                    
                    // Update Tension Monitor (Heuristic: Variance in alignment/skepticism)
        ...
        <div className="flex h-full bg-[#050505] relative overflow-hidden">
            ...
            <div className="w-1/2 flex flex-col relative border-r border-[#1f1f1f] p-0 overflow-hidden">
                ...
                <div className="absolute top-0 left-0 right-0 p-6 flex justify-between items-start z-20 pointer-events-none">
                    ...
                    <button 
                        onClick={toggleVoice}
                        className={`pointer-events-auto flex items-center gap-2 px-3 py-1.5 rounded border transition-all text-[10px] font-mono uppercase tracking-wider ${isVoiceActive ? 'bg-[#9d4edd]/20 border-[#9d4edd] text-[#9d4edd] shadow-[0_0_15px_rgba(157,78,221,0.2)]' : 'bg-[#111] border-[#333] text-gray-500 hover:text-white'}`}
                    >
                        {isVoiceActive ? <Radio className="w-3 h-3 animate-pulse" /> : <Mic className="w-3 h-3" />}
                        {isVoiceActive ? 'VOICE LINK ACTIVE' : 'VOICE MUTED'}
                    </button>
                </div>
                                            ...
                                            >
                                                ...
                                                {isActive && (
                                                    <>
                                                        ...
                                                        {isVoiceActive && (
                                                            <div className="absolute inset-[-10px] border border-[var(--glow)] rounded-full animate-ping opacity-50"></div>
                                                        )}
                                                    </>
                                                )}
                                            </div>
                ...
                <div className="h-12 border-b border-[#1f1f1f] flex items-center justify-between px-4 bg-[#0a0a0a]">
                    ...
                    {status === 'DEBATING' && (
                        <div className="flex items-center gap-2 px-2 py-1 bg-[#1f1f1f] rounded border border-[#333]">
                            ...
                            <span className="text-[9px] text-gray-500 font-mono">Right-click avatar to whisper</span>
                        </div>
                    )}
                </div>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-4" ref={scrollRef}>
                    {history.map((turn) => {
                        ...
                        return (
                            <motion.div 
                                ...
                                className="text-xs font-mono flex flex-col gap-1"
                            >
                                ...
                                <div className="bg-[#111] border border-[#222] p-3 rounded-tr-lg rounded-b-lg text-gray-300 leading-relaxed relative">
                                    {turn.newMindset && (
                    ...
                    {status === 'DEBATING' && (
                        <div className="flex items-center gap-2 text-xs text-gray-500 animate-pulse mt-4 pl-2">
                            ...
                            <span className="font-mono">Deliberating next move...</span>
                        </div>
                    )}
                </div>
