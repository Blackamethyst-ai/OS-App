...
describe('ApiUsageService', () => {
    ...
    describe('recordCall', () => {
        it('increments totalCalls on each record', () => {
            apiUsageService.recordCall('gemini-2.0-flash', true);
            apiUsageService.recordCall('gemini-2.0-flash', true);
        ...
        it('tracks errors separately from successes', () => {
            apiUsageService.recordCall('gemini-2.0-flash', true);
            apiUsageService.recordCall('gemini-2.0-flash', false);
        ...
        it('groups calls by model', () => {
            ...
            expect(callsByModel['gemini-2.0-flash']).toBe(2);
    ...
    describe('getRateLimitInfo', () => {
        it('returns correct limits for known model', () => {
            const info = apiUsageService.getRateLimitInfo('gemini-2.0-flash');
    ...
    describe('isRateLimited', () => {
        it('returns false when under limit', () => {
            apiUsageService.recordCall('gemini-2.0-flash', true);
            expect(apiUsageService.isRateLimited('gemini-2.0-flash')).toBe(false);
        });
        ...
        it('returns true when at limit', () => {
            ...
            expect(apiUsageService.isRateLimited('gemini-2.0-flash')).toBe(true);
        });
    });
    ...
    describe('model normalization', () => {
        it('normalizes gemini-2.0-flash variants to same key', () => {
            apiUsageService.recordCall('gemini-2.0-flash-exp', true);
            const { callsByModel } = apiUsageService.getStats();
            expect(callsByModel['gemini-2.0-flash-exp']).toBe(1);
        });
