...
function getModelForPath(path: CPBPath, forceModel?: ReasoningModel): { provider: 'gemini' | 'claude'; model: string } {
    if (forceModel && forceModel !== 'auto') {
        switch (forceModel) {
            case 'gemini-flash':
                return { provider: 'gemini', model: MODEL_REGISTRY.gemini.fast };
            case MODEL_REGISTRY.gemini.fast:
                return { provider: 'gemini', model: MODEL_REGISTRY.gemini.fast };
            case 'claude-haiku':
    ...
    switch (path) {
        case 'direct':
            ...
            return claudeService.isConfigured()
                ...
                : { provider: 'gemini', model: MODEL_REGISTRY.gemini.fast };
        case 'rlm':
            ...
            if (images && images.length > 0) {
                ...
                const response = await retryGeminiRequest(() =>
                    ai.models.generateContent({
                        model: MODEL_REGISTRY.gemini.fast, // Flash supports vision
                        contents: contents,
